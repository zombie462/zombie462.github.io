'use strict';
{
	...
	const VERSION_NAME='';
	const VERSION_MESSAGE=`调整了界面，增加了大量新成就。`;
	const {encode,decode}=(()=>{
		const C=126-33+1;
	...
	const LENSTAR='<span class="LENSTAR">LENSTAR</span>';
	const DAILY_MESSAGES=[
		`你强归你强，${LENSTAR}比你强！`,
		`你可以按住<code>Enter</code>来快速点击一个按钮。`,
		`${LENSTAR}是我们的红太阳，没有她我们会死！`,
		`你<strong>不能</strong>通过按住<code>Shift</code>点击一个按钮以直接购买最大数量的事物。`,
		`今天又是美好的一天~`,
		`若不是${LENSTAR}相助，也许我到现在还不知道我做错了什么。现在我知道了，是<code>LCT</code>上<code>splay</code>的时候判断父亲是否为根直接用了<code>if(fa->fa)</code>。`,
		`洁白的光点游走于漆黑的天幕之上，暗紫色的薄纱在其间舞动。在此无月之夜，是否可以知晓：“后缀自动机怎么写？”`,
		`那些文档有一部分真的有用……`,
		`${LENSTAR} 膜 她 自 己`,
		`${LENSTAR} D 她 自 己`,
		`如果你花掉了一些东西，它的花费也会降低，就像你从没买过它一样。`,
		`要想探寻真理，不仅要虔诚膜拜${LENSTAR}，领导者的实力也是很重要的。`,
		`<code>code.replace(/\\n\\s*\\{/mg,\'{\');</code>`,
		`嘤嘤嘤`,
		`Hello world!`,
		`在夕阳的照耀下，教堂的圆顶散发出金色的光辉。`,
		`这一刻，我们都是${LENSTAR}的忠实粉丝。`,
		`Do the magic!`,
		`传说${LENSTAR}会在被她抛弃的终端上用 root 权限执行<code>rm -rf /*</code>。`,
		`${LENSTAR}太强了，所以你复制不了这个网站中的任何一个${LENSTAR}。`,
		`空谈误国，实干兴邦。`,
		`Orz${LENSTAR} 就来 <a href="https://www.cnblogs.com/Lenstar">Lenstar的博客</a>！`,
		`追寻真理的各种花费与你本轮尝试次数有关。`,
		`追寻真理时重置会将当前成功轮数也清空！`,
		`在真理之路上，要不畏艰辛，敢于推翻重来，才不会在错误的道路上越走越远。`,
		`窝 又 被 ${LENSTAR} D 了 QAQ`,
		`道路千万条，光明第一条。防御没做好，黑屏两行泪。`,
		`元素和光明在你下线时也会增加！`,
		`贪 D 的(9^0+9^1+...+9^n+...)头 ${LENSTAR}`,
		`${LENSTAR}:“辣鸡，真辣鸡！”`,
		`${LENSTAR}:“泥萌怎么这么菜 nya？”`,
		`${LENSTAR}:“我就 D 你怎么了？”`,
		`${LENSTAR}:“哪场 ** CF 连样例解释都没有？”`,
		`${LENSTAR}:“tourist 能过，那窝肯定也能过！”`,
		`萌新三连:“窝怎么立直了 nya？胡是什么 nya，可以跳过吗？自摸是不是每巡都有的，好烦 nya！”`,
		`如果出现了难以对付的敌人，你可以<strong>召唤拯救</strong>扫清他们。`,
		`2019-08-21 ${LENSTAR} 被阿祭`,
		`${LENSTAR} AK NOI！`,
		`${LENSTAR} AK IOI！`,
		`${LENSTAR} 女装！`,
		`<strong>传教</strong>可以增加你的信徒数量。`,
		`传教中出现的题目来自某场${LENSTAR}将要 AK 的比赛的笔试。`,
		`强迫症真的这么好吗？`,
	...
	const SP_PROBLEMS=[
		...
		['在 Linux 系统中，文件夹中的文件可以与该文件夹同名吗：可以','不行',`只有${LENSTAR}才能做到`,'我不知道'],
		...
		[`使用 Reset 键强行重新启动计算机可能会对系统造成的后果是：文件系统损坏、膜拜${LENSTAR}存档损坏`,'中央处理器损坏','硬盘损坏'],
...
-rwx------ 1 user None 144  Jul 2 16:35 foobar.sh
	...
	其中，可以执行的文件是：foobar.sh`,
		...
		['选手在 NOI 机试过程中是否禁止使用网络：是','否',`只有${LENSTAR}可以`,'使用前需向监考人员举手示意'],
		['为程序 my.c 创建一个备份 myc.bak 时，使用的命令是：cp my.c myc.bak','he my.c my.bak','mv -c my.bak my.c','copy my.c my.bak'],
		['调试程序的方法有：单步调试、使用 print 类语句打印中间结果、读源代码','向监考人员求助',`膜拜${LENSTAR}以寻求帮助`],
		['在考试过程中，如果出现系统死机或者崩溃现象，选手应当采取的措施是：举手示意监考人员处理','关机重启',`向${LENSTAR}祈祷`,'使用旁边的电脑'],
		['提交的答案程序中如果包含 NOI 考试明确禁止使用的代码，后果是：本题成绩以 0 分计算','本场比赛成绩以 0 分计算','受到禁赛处罚','以评测机分数为准'],
		['对评测结果有疑义，需要申请复评，则：提出书面申请；经领队、科学委员会主席签字确认后，提交至评测人员','在调整程序之后',`在膜拜${LENSTAR}之后`],
		...
		['在 NOI 正式考试中如何登录自己的比赛用机：使用考前工作人员下发的账户及密码',`使用膜拜${LENSTAR}的账户及密码`,'使用试机时使用的账户及密码','使用考试开始时公布的账户及密码'],
		...
		['对试题理解有问题，选手可以：举手求助','将错就错',`膜拜${LENSTAR}`,'谨慎猜测'],
		...
		['草稿纸用完了，如何处理：举手向监考人员求助','使用自带的草稿纸','直接将草稿打在桌面上',`膜拜${LENSTAR}`],
		['水喝完了，如何处理：举手向监考人员再要一瓶','到饮水机处自行灌水','渴着','从其他选手处得到水瓶'],
		[`${LENSTAR}觉得考试太简单，能提前离开吗？能`,'不能','将题目全部完成方可离场',`只有${LENSTAR}能提前离场`],
		[`${LENSTAR}离开考场后，发现有个题可以吊打标算，能回去再改吗？不能`,'能','向监考人员说明情况后可返回考场','向科学委员会提交书面申请并通过后可返回考场'],
		...
		['IT 表示：信息技术','工业技术','电脑技术',`膜拜${LENSTAR}技术`],
		...
		['现代计算机所应用的存储程序原理是谁提出的？冯·诺依曼','艾伦·麦席森·图灵',`${LENSTAR}`,'Gennady Korotkevich'],
	...
	const TECH={
		1:{
			optics:{
				name:`光学`,
				description:`水晶的获取使光学成为最先被发展的科学学科。（增加每次祈祷获得的水晶数）`,
				require:[],
				cost(lv){
					return [
			...
			focus:{
				name:`聚焦`,
				description:`用水晶打磨成镜片，利用阳光增幅信仰获取。（增加膜拜至信仰的转化率）`,
				require:[['optics',1]],
				cost(lv){
			...
			magnifier:{
				name:`放大镜`,
				description:`让你的魔法师不用超凡的眼力也能弄清魔法石内部的魔力结构。（增加魔力的获取速度）`,
				require:[['optics',2]],
				cost(lv){
			...
			glasses:{
				name:`眼镜`,
				description:`黑色的框架配上镜片，足以使老龄科学家与神学家看得更清楚。（增加神学获取速度、研究获取速度和研究上限）`,
				require:[['optics',3]],
				cost(lv){
					return [
			...
			xuanxue:{
				name:`玄学`,
				description:`经过她手里的随机事件便受她掌控，但没被她看到的呢？玄学便是为了探究这个问题而生。（除了解锁其它真理没有实质用途）`,
				require:[],
				cost(lv){
					return [
			...
			dunai:{
				name:`毒奶`,
				description:`通过反向毒奶可以减少实验时不该产生的误差，进而减少水晶消耗。（减少探究真理的水晶消耗）`,
				require:[['xuanxue',3]],
				cost(lv){
					return [
			...
			tidy:{
				name:`整洁`,
				description:`洗个澡，把衣服穿戴整齐，往往运气会更好！效果与真诚膜拜等级的质因数分解中 2 的数量有关。（大幅度减少信徒花费，取决于真诚膜拜等级的质因数分解中 2 的数量）`,
				require:[['xuanxue',4]],
				cost(lv){
			...
			spell:{
				name:`法术`,
				description:`使用不了法术，还能称之为魔法吗？（增加每次召唤信徒的数量）`,
				require:[],
				cost(lv){
					return [
						['magicStone',500*(lv+1)],
			...
			spellWater:{
				name:`流水术`,
				description:`江河湖海，皆在掌控。（减少探究真理的研究、神学和魔力的花费）`,
				require:[['spell',2]],
				cost(lv){
			...
			devotionInduction:{
				name:`虔诚感应`,
				description:`一束玄妙的魔力丝通向天空，让她知道你正在认真地膜拜她。（激活感应系统并增加感应效果）`,
				require:[
					['focus',1],
					['spell',4],
				],
		...
		2:{
			pscience:{
				name:`科普`,
				description:`用一些不科学的小东西反而能唤起孩子们对科学的好奇。（研究获取速度和研究上限）`,
				require:[],
				cost(lv){
					return [
			...
			geometry:{
				name:`几何学`,
				description:`花纹与光斑启发了人们对图形的思考。（除了解锁其它真理没有实质用途）`,
				require:[
					['focus',5],
					['tidy',1],
				],
			...
			hugeStoneBuilding:{
				name:`巨石`,
				description:`她是如此之巨，以至于巨石都听从她的号令！（减少巨石花费）`,
				require:[['geometry',1]],
				cost(lv){
					return [
						['theology',1e6*(lv+2)**2],
			...
			spellWind:{
				name:`风语术`,
				description:`隐匿于气，交谈于风。（除了解锁其它真理和魔法塔没有实质用途）`,
				require:[['spell',6]],
				cost(lv){
			...
			spellBird:{
				name:`御鸟术`,
				description:`与鸟相关？对了一半。这法术确实与麻雀相关……（除了解锁其它真理和魔法塔没有实质用途）`,
				require:[
					['spell',8],
					['spellWind',3],
				],
			...
			fazhenBuilding:{
				name:`法阵`,
				description:`将施法过程描述出来，静态化，就变成了法阵。（增加魔力的获取速度）`,
				require:[
					['geometry',1],
					['spell',10],
				],
			...
			fireFazhen:{
				name:`烈焰阵`,
				description:`燃烧！不过好像没有什么东西该被烧掉呢。（除了解锁其它真理和魔法塔没有实质用途）`,
				require:[
					['geometry',3],
					['fazhenBuilding',2],
				],
			...
			explore:{
				name:`探索`,
				description:`城镇的远处时常有微光传来，那似乎不仅仅是自然的造物……（除了解锁其它真理没有实质用途）`,
				require:[
					['fireFazhen',2],
					['optics',12],
				],
			...
			warMindInduction:{
				name:`战意感应`,
				description:`法阵与乌云以红色的光柱连接，让她知道人们渴望战斗。（激活战争系统并增加战争效果）`,
				require:[
					['explore',3],
					['fireFazhen',5],
					['devotionInduction',5],
			...
			windFazhen:{
				name:`疾风阵`,
				description:`强劲的风力加上成块的碎土能使敌人倒退好几步，但也会吹跑正在飞向教堂的鸽子。（除了解锁其它真理和魔法塔没有实质用途）`,
				require:[
					['explore',3],
					['fireFazhen',5],
					['spellBird',8],
		...
		3:{
			antiGugu:{
				name:`驱鸽仪`,
				description:`有人曾说这将是真理 III 中的第一个研究。现在看来他说得对。（除了解锁其它真理和魔法塔没有实质用途）`,
			...
			blessing:{
				name:`祝福术`,
				description:`来自她的祝福能让信徒们如获新生。（除了解锁其它真理【暂时】没有实质用途）`,
				require:[
					['antiGugu',1],
					['windFazhen',2],
					['explore',4],
			...
			timeInduction:{
				name:`时间感应`,
				description:`数百双眼睛盯着法阵中幽蓝色的光芒，向她祈求永恒的瞬间。（激活时间系统并增加时间效果）`,
				require:[
					['antiGugu',2],
					['fazhenBuilding',9],
					['glasses',40],
			...
			magicTree:{
				name:`魔灵树`,
				description:`魔灵树的果实天生自带保护魔法，这使这些果实为敌人所觊觎。（减少魔灵树的花费）`,
				require:[
					['blessing',10],
					['fazhenBuilding',10],
				],
			...
			industry:{
				name:`工业`,
				description:`这是由科技和魔法共同组成的工业。（大幅度增加研究增长速度）`,
				require:[
					['spell',15],
					['geometry',8],
				],
				cost(lv){
					return [
						['science',300000*Math.pow(2,lv)],
						['magic',3000000*Math.pow(2,lv)],
					];
				},
			},
			potion:{
				name:`酿造术`,
				description:`通过酿造人们找到了快速增加魔力的方法。（增加魔力增长速度）`,
				require:[
					['spell',15],
					['blessing',15],
				],
				cost(lv){
					return [
						['magic',60000000*Math.pow(2,lv)],
					];
				},
			},
			enchant:{
				name:`附魔术`,
				description:`有了附魔效果的经书将会得到很大的提升。（加强经书效果）`,
				require:[
					['spell',20],
					['xuanxue',9],
				],
			...
			lucky:{
				name:`幸运`,
				description:`幸运使得人们在祈求神时，神会给予他们额外的恩赐。（祈求恩赐时有概率得到额外的水晶）`,
				require:[
					['spell',20],
					['xuanxue',11],
					['devotionInduction',15], 
				],
				cost(lv){
					return [
						['magic',50000000*Math.pow(2,lv)],
						['theology',50000000*Math.pow(2,lv)],	
						['crystal',lv*700],					
					];
				},
			},
		...
		4:{
			yuanquan:{
				name:`法力源泉`,
				description:`有人曾说这将是真理 IV 中的第一个研究。现在看来他说得对。（大幅度减少法阵的花费）`,
				require:[['hugeStoneBuilding',20]],
				cost(lv){
					return [
						['magic',Math.pow(2,lv)*3000000000],
						['altar',lv+3],
					];
				},
			},
			greatatom:{
				name:`元素混合`,
				description:`科技使得不同的元素能够混合起来，以达到各取其长的目的。（除了解锁魔法塔没有实质用途）`,
				require:[
					['fireFazhen',5],
					['windFazhen',2],
					['fazhenBuilding',5],
				...
				cost(lv){
					return [
						['hugeStone',5*lv],
						['magic',Math.pow(2,lv)*300000000],
					];
				},
			},
			computer:{
				name:`电子技术`,
				description:`第三次工业革命使得研究的上限得到增加。（大幅度增加研究增长上限）`,
				require:[
					['spell',15],
					['geometry',8],
					['industry',2],
				...
				cost(lv){
					return [
						['science',3000000*Math.pow(2,lv)],
						['magic',30000000*Math.pow(2,lv)],
					];
				},
			},
			merging:{
				name:`融合术`,
				description:`人们发现了将不同种类魔法融合的办法，巨量的魔力随之而来。（大幅度增加魔力增长速度）`,
				require:[
					['spell',20],
					['blessing',18],
				],
				cost(lv){
					return [
						['magic',200000000*Math.pow(2,lv)],
					];
				},
			},
			ultiwisdom:{
				name:`终极智慧`,
				description:`人们用智慧得到了额外的眷顾，使得祈求恩赐变得更简易。（大幅度减少水晶花费）`,
				require:[
					['computer',5],
					['merging',3],
				],
				cost(lv){
					return [
						['science',30000000*Math.pow(2,lv)],
						['magic',300000000*Math.pow(2,lv)],
						['theology',300000000*Math.pow(2,lv)],
					];
				},
			},
		},
		5:{
			atomcomputer:{
				name:`量子物理`,
				description:`这是科技、魔法和信仰的一次融合。（除了解锁其它真理没有实质用途）`,
				require:[
					['spell',20],
					['xuanxue',10],
					['geometry',18],
			...
			universe:{
				name:`天文学`,
				description:`天文学的研究使得人们的视野开阔至宇宙深处，人们对神学有了更深的认识。（大幅度增加神学增长速度）`,
				require:[
					['atomcomputer',3],
				],
				cost(lv){
					return [
						['science',50000000*Math.pow(2,lv)],
						['magic',500000000*Math.pow(2,lv)],
						['theology',500000000*Math.pow(2,lv)],
					];
				},
			},
			outerspace:{
				name:`航天技术`,
				description:`带有魔法燃料的飞船可以飞得更远。（除了解锁其它真理没有实质用途）`,
				require:[
					['universe',6],
				],
				cost(lv){
					return [
						['science',100000000*Math.pow(2,lv)],
						['magic',1000000000*Math.pow(2,lv)],
					];
				},
			},
			planetexplore:{
				name:`外星探索`,
				description:`一艘艘飞船带回了外星物资，包括来自外星的水晶。（水晶数将缓慢地自动提升）`,
				require:[
					['outerspace',5],
				],
				cost(lv){
					return [
						['crystal',250*Math.pow(2,lv)],
						['science',1000000000*Math.pow(2,lv)],
						['magic',1000000000*Math.pow(2,lv)],
					];
				},
			},
	...
	const TRUTH_UPGRADES={
		...
		5:{
			...
			dis(x,y,z,tx,ty,tz){
				...
				let dy=Math.abs(y-ty);
				let dz=Math.abs(z-tz);
				return (dx+dy+dz)+Math.floor(this.random('truthUpgrade')*11-5);
			},
			message(res){
				return `模糊距离:${res}`;
			},
	...
	const DEFENSE_BUILDING={
		...
		atomTower:{
			name:`混合元素塔`,
			description:['2魔法伤害(0.1s冷却)','水、火、土、风、魔元素各0.03/秒'],
			require:{
				tech:[['greatatom',3]],
				element:['magic'],
			},
	...
	const ACHIEVEMENTS={
		...
		greatbusyWork:{
			name:`无尽繁忙`,
			description:`拥有 2000 座教堂`,
			got:(game)=>game.churchs>=2000,
		},
		tinyWork:{
			name:`微小的贡献`,
		...
		sp5:{
			name:`超常发挥`,
			description:`达到传教 V`,
			got:(game)=>game.spLevel>=5,
		},
		middleWork:{
			name:`中等的贡献`,
			description:`积累 1B 信仰`,
			got:(game)=>game.XY>=1000000000,		
		},
		greatWork:{
			name:`巨大的贡献`,
			description:`积累 1T 信仰`,
			got:(game)=>game.XY>=1000000000000,		
		},
		giantWork:{
			name:`顶级的贡献`,
			description:`积累 1q 信仰`,
			got:(game)=>game.XY>=1000000000000000,		
		},
		unlessWork:{
			name:`无尽的贡献`,
			description:`积累 1s 信仰`,
			got:(game)=>game.XY>=1000000000000000000000,		
		},
		ultimateWork:{
			name:`终极的贡献`,
			description:`积累 1O 信仰`,
			got:(game)=>game.XY>=1000000000000000000000000000,		
		},
		sp10:{
			name:`终极发挥`,
			description:`达到传教 IX`,
			got:(game)=>game.spLevel>=9,	
		},
		th1:{
			name:`透过迷雾`,
			description:`达到真理 I`,
			got:(game)=>game.truthLevel>=1,	
		},
		th2:{
			name:`执着追寻`,
			description:`达到真理 II`,
			got:(game)=>game.truthLevel>=2,		
		},
		th3:{
			name:`显现奥秘`,
			description:`达到真理 III`,
			got:(game)=>game.truthLevel>=3,		
		},
		th4:{
			name:`豁然开朗`,
			description:`达到真理 IV`,
			got:(game)=>game.truthLevel>=4,	
		},
		th5:{
			name:`最终真理`,
			description:`达到真理 V`,
			got:(game)=>game.truthLevel>=5,	
		},
		getcrystal:{
			name:`${LENSTAR}的恩赐`,
			description:`得到水晶`,
			got:(game)=>game.crystal>=1,	
		},
		moer1:{
			name:`膜拜者`,
			description:`拥有1位信徒`,
			got:(game)=>game.moers>=1,	
		},
		moer2:{
			name:`追随者`,
			description:`拥有1000位信徒`,
			got:(game)=>game.moers>=1000,	
		},
		moer3:{
			name:`共同的信仰`,
			description:`拥有10000位信徒`,
			got:(game)=>game.moers>=10000,	
		},
		knowledge1:{
			name:`知识海洋`,
			description:`拥有1本知识之书`,
			got:(game)=>game.knowledgeBook>=1,	
		},
		knowledge2:{
			name:`知识宇宙`,
			description:`拥有2本知识之书`,
			got:(game)=>game.knowledgeBook>=2,	
		},
		th6:{
			name:`究极真理`,
			description:`达到真理 VI`,
			got:(game)=>game.knowledgeBook>=6,	
		},
	};
		...
		new Vue({
			...
			methods:{
				...
				pray(){
					...
					this.crystal+=1+7*Math.min(this.tech.optics,5)+Math.floor(Math.random()*70*this.tech.lucky);
				},
				wisdomUpgrade(){
				...
				gameTick(s){
					...
					this.crystal+=s*this.tech.planetexplore*10; 
			...
			computed:{
				...
				bookEffect(){
					return (Math.min(this.moCount/1000,1)+Math.floor(this.books*1.2*Math.pow(1+this.mysteryLevel,1.5)))*(this.tech.enchant/2+1);
				},
				...
				prayCost(){
					return Math.pow(1.6,this.crystal/this.temple/(1+this.truthLevel))*1e10/Math.pow(this.XY,1/3)/Math.pow(6,this.tech.ultiwisdom);
				},
				canUpgradeWisdom(){
				...
				makeGemEarn(){
					return Math.max(0,Math.floor(this.crystal/3*(this.wisdomLevel/5)-this.gem*0.6));
				},
				makeMagicStoneEarn(){
					return Math.max(0,Math.floor(this.crystal/3*(this.mysteryLevel/5.5)-this.magicStone*0.6));
				},
				makeLenEarn(){
				...
				theologyPerSec(){
					return Math.max(0,Math.sqrt(this.moDelta*this.gem)*Math.pow(2,this.tech.universe)-(1+this.theology)/this.altar)/1e4*(1+this.tech.glasses);
				},
				magicianCost(){
				...
				magicRate(){
					return 5*this.magician*(1+this.tech.magnifier/4)*this.fazhenEffectFactor*Math.pow(1.2,this.tech.potion)*Math.pow(1.4,this.tech.merging);
				},
				scientistCost(){
					return 16*Math.pow(1.5,this.scientist);
				},
				sciencePerSec(){
					return Math.max(0,Math.sqrt(this.len*this.scientist)/2)*Math.sqrt(1+this.tech.glasses)*(1+Math.sqrt(this.tech.pscience)/4)*Math.pow(1.9,this.tech.industry);
				},
				scienceLimit(){
					return 50*Math.pow(this.scientist,2)*(1+this.tech.glasses)*(1+Math.sqrt(this.tech.pscience)/4)*Math.pow(1.9,this.tech.computer);
				},
				...
				fazhenCost(){
					return Math.floor(3*Math.pow(this.fazhen+1,2)/(1+this.tech.yuanquan*0.4));
				},
				canBuyKB(){
