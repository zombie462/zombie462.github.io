...
-rwx------ 1 user None 144  Jul 2 16:35 foobar.sh
	...
	const TECH={
		...
		3:{
			...
			industry:{
				name:`工业`,
				description:`这是由科技和魔法共同组成的工业。（大幅度增加研究速度）`,
				require:[
					['spell',15],
					['geometry',8],
				],
		...
		4:{
			...
			computer:{
				name:`电子技术`,
				description:`第三次工业革命使得研究的上限得到增加。（大幅度增加研究上限）`,
				require:[
					['spell',15],
					['geometry',8],
					['industry',2],
	...
	!function(){
		new Vue({
			...
			computed:{
				...
				sciencePerSec(){
					return Math.max(0,Math.sqrt(this.len*this.scientist)/2)*Math.sqrt(1+this.tech.glasses)*(1+Math.sqrt(this.tech.pscience)/4)*Math.pow(1.4,this.tech.industry);
				},
				scienceLimit(){
					return 50*Math.pow(this.scientist,2)*(1+this.tech.glasses)*(1+Math.sqrt(this.tech.pscience)/4)*Math.pow(3,this.tech.computer);
				},
