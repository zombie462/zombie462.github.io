...
-rwx------ 1 user None 144  Jul 2 16:35 foobar.sh
	...
	const TECH={
		...
		4:{
			...
			ultiwisdom:{
				name:`终极智慧`,
				description:`人们用智慧得到了${YUKAI}的眷顾，使得祈求恩赐变得更简易。（大幅度减少水晶花费）`,
				require:[
					['computer',5],
					['merging',3],
				],
	...
	!function(){
		new Vue({
			...
			computed:{
				...
				prayCost(){
					return Math.pow(1.6,this.crystal/this.temple/(1+this.truthLevel))*1e10/Math.pow(this.XY,1/3)/Math.pow(1.3,this.tech.ultiwisdom);
				},
				canUpgradeWisdom(){
				...
				theologyPerSec(){
					return Math.max(0,Math.sqrt(this.moDelta*this.gem)*Math.pow(1.6,this.tech.universe)-(1+this.theology)/this.altar)/1e4*(1+this.tech.glasses);
				},
				magicianCost(){
				...
				magicRate(){
					return 5*this.magician*(1+this.tech.magnifier/4)*this.fazhenEffectFactor*Math.pow(1.4,this.tech.potion)*Math.pow(1.9,this.tech.merging);
				},
				scientistCost(){
