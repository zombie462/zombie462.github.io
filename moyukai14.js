...
-rwx------ 1 user None 144  Jul 2 16:35 foobar.sh
	...
	const TECH={
		...
		3:{
			...
			potion:{
				name:`酿造术`,
				description:`通过酿造人们找到了如何快速增加魔法的方法。（增加魔力增长速度）`,
				require:[
					['spell',15],
					['blessing',15],
				],
	...
	!function(){
		new Vue({
			...
			computed:{
				...
				theologyPerSec(){
					return Math.max(0,Math.sqrt(this.moDelta*this.gem)-(1+this.theology)/this.altar)/1e4*(1+this.tech.glasses)*Math.pow(1.6,this.tech.universe);
				},
				magicianCost(){
				...
				magicRate(){
					return 5*this.magician*(1+this.tech.magnifier/4)*this.fazhenEffectFactor*Math.pow(1.4,this.tech.potion);
				},
				scientistCost(){
